  ...
  <v-container>
        ...
        >
          <template v-slot:item.action="{item}">
            <v-btn small color="primary" @click="openAssign(item.id, item.name)">
              <v-icon small left>swap_horiz</v-icon>Set Territory
            </v-btn>
          </template>
        </v-data-table>
      </v-row>
    ...
    <v-dialog v-model="dialogAssign" width="300" :persistent="true">
      <v-card :loading="tableLoad">
        <v-card-title>
          <p class="text-capitalize">Change Territory</p>
        </v-card-title>
        <v-card-text>Change Client Territory to {{leftName}}</v-card-text>
        <v-card-actions>
