<template>
  <v-container grid-list-xs>
    <v-row>
      ...
      <v-col md="8" xs="12">
        <v-btn color="primary" @click="openAdd">
          <v-icon left>add</v-icon>Add Statistic Report
        </v-btn>
      </v-col>
    </v-row>
    <v-row>
    ...
    <report-form v-if="dialogForm" :edit="edit" @close="dialogForm = false" @refresh="refresh" />
...
export default {
  ...
  methods: {
    ...
    deleteAccount(id) {
      this.tableLoad = true;
      this.axios
        .delete(config.baseUri + "/statistic_reports/" + id, {
          headers: auth.getAuthHeader()
        })
        .then(res => {
          this.refresh();
